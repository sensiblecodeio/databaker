Feature: Define dimension using bag.is_number()
    ...
    def transform_xlsx():
        ...
        return tabs
    """
    And we define the observations as all values between cell refs "C8" and "K57" excluding the non-GBP values.
    Then we confirm the observations contains the correct number of values: "48"
...
Scenario: Define the observations as correct the cells.
    ...
    """
    And we define the observations as all values between cell refs "C8" and "K57" excluding the non-GBP values.
    Then we confirm that the observations are equal to:
    """
