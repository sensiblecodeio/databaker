Feature: Define a dimension as a range of values from a cell reference but only including the cells containing whitespace.
  ...
  Scenario: Define whitespace year from a range of cell references ignoring any value-containing cells.
    ...
    def transform_xlsx():
        ...
        return tabs
    """
    And we define whitespace year as the whitespace cells in the range "A11:A250"
    Then we confirm whitespace year contains no value-containing cells.
...
Scenario: Define whitespace year as correct cells.
    ...
    """
    And we define whitespace year as the whitespace cells in the range "A11:A250" 
    Then we confirm that whitespace year without value-containing cells is equal to:
    """
