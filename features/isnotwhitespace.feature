Feature: Define a dimension as a range of values from a cell reference but not including the cells containing whitespace.
  ...
  Scenario: Define non-whitespace year from a range of cell references ignoring any whitespace cells.
    ...
    def transform_xlsx():
        ...
        return tabs
    """
    And we define non-whitespace year as the non-whitespace values in cells "A11:A250"
    Then we confirm non-whitespace year contains no whitespace cells.
    And we confirm non-whitespace year contains "58" cells.
...
Scenario: Define non-whitespace year as correct cells.
    ...
    """
    And we define non-whitespace year as the non-whitespace values in cells "A11:A250" 
    Then we confirm that non-whitespace year without whitespace is equal to:
    """
