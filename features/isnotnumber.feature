Feature: Define dimension using bag.is_non_number()
    ...
    """
    def transform_xlsx():
        tabs = [x for x in tabs if x.name.strip() == "Records"]
        return tabs
    """
    And we define the string observations as all values between cell refs "C8" and "K57" excluding the GBP values.
    Then we confirm the string observations contains the correct number of values: "48"
...
Scenario: Define the observations as correct the cells.
    ...
    """
    And we define the string observations as all values between cell refs "C8" and "K57" excluding the GBP values.
    Then we confirm that the string observations are equal to:
    """
